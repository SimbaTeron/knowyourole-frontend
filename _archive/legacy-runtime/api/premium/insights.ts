...
export const POST = requireAuth(async (req: Request, ctx: { user: { sub: string } }) => {
  try {
    ...
    if (!mbtiType || !bigFive) {
      return NextResponse.json({ error: 'mbtiType and bigFive are required' }, { status: 400 });
    }
    ...
    return NextResponse.json({
      ...
      message: 'Auth0 not wired — using placeholder insights. Wire Auth0 + seed premium_insights table to activate.',
    });
  } catch (error) {

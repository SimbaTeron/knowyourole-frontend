...
export async function registerRoutes(
  ...
  app.post("/api/share/email", async (req: Request, res: Response) => {
    try {
      ...
      const { PDFDocument, rgb, StandardFonts } = await import("pdf-lib");
      const { email, sessionId, result, mood, tier } = req.body;
      ...
      if (!emailUser || !emailPass) {
        return res.status(500).json({ error: "Email service not configured. Please set EMAIL_USER and EMAIL_PASS." });
      }
  ...
  app.post("/api/share/sms", async (req: Request, res: Response) => {
    try {
      const { phone, sessionId, result } = req.body;
      ...
      if (!twilioSid || !twilioToken || !twilioPhone) {
        return res.status(500).json({ error: "SMS service not configured. Please set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, and TWILIO_PHONE_NUMBER." });
      }

      const twilio = await import("twilio");
      const client = twilio.default(twilioSid, twilioToken);

      const appUrl = process.env.REPLIT_DOMAINS?.split(",")[0] || "knowrole.replit.app";
      const resultUrl = `https://${appUrl}/results?session=${sessionId}`;

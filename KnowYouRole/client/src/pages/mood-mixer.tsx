...
export default function MoodMixer() {
  ...
  // Guard: if no tier, set default and allow access to mood mixer
  useEffect(() => {
  ...
  // Dev/test mode: auto-jump to quiz questions (skip mood selection)
  // Only fires when ?test=true is explicitly in URL
  useEffect(() => {
    if (new URLSearchParams(window.location.search).get("test") !== "true") return;
    // Set a default mood blend for test results
    const blendName = "Focused + Curious Blend";
    localStorage.setItem("kyr_mood_blend", JSON.stringify({
      mood1: "focused", mood2: "curious", label: blendName,
    }));
    sessionStorage.setItem("knowrole-mood-blend", blendName);
    // Auto-jump to quiz questions
